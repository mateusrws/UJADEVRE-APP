...
// TODO: Replace the entire mockTickets lookup with checkInTicket() from registrationService.ts
// TODO: Call checkInTicket(ticketId) on scan, then display the result from the backend response
// TODO: The backend should handle validation, duplicate check-in prevention, and state persistence
// TODO: Remove the mockTickets constant entirely when backend is ready
...
export function CheckIn() {
    ...
    const handleScan = () => {
        ...
        if (!ticket.valid) {
            setScanResult({ type: 'error', message: 'Ingresso inválido ou cancelado', ticket });
            return;
        }

        if (ticket.checkedIn) {
            setScanResult({ type: 'warning', message: 'Check-in já realizado anteriormente', ticket });
            return;
        }

        setTickets({ ...tickets, [ticketId]: { ...ticket, checkedIn: true } });
        setScanResult({
            type: 'success',
    ...
    return (
        <div className="flex flex-col gap-6 p-6 bg-gray-50 min-h-full">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
                <label className="text-sm text-gray-600 mb-3 block">Digite o código do ingresso</label>
            ...
            {scanResult && (
                <div className={`rounded-2xl p-6 shadow-sm ${
                    scanResult.type === 'success' ? 'bg-green-50 border-2 border-green-200'
                    : scanResult.type === 'warning' ? 'bg-yellow-50 border-2 border-yellow-200'
                    ...
                    {scanResult.ticket && (
                        <div className="bg-white rounded-xl p-4 space-y-2">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-semibold">{new Date(scanResult.ticket.date).toLocaleDateString('pt-BR')}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">Status:</span>
                                <span className={`font-semibold ${scanResult.ticket.valid ? 'text-green-600' : 'text-red-600'}`}>
